...
async def on_message(message):
    ...
    while message.author != client.user and new_message == True and not message.author.bot:
        ...
        # character search
        elif scannedResult.find('Code 7') != -1:
            await message.channel.send(embed=botFunctions.searchCharacterInfo(botFunctions, user_input))
            new_message = False
            break
